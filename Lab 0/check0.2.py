...
def has_two_double(word):
    for l in range(len(word)-3):
        if word[l] == word[l+1] and word[l+2] == word[l+3]:
            return True
    return False

def has_three_double_split1(word):
    for l in range(len(word)-6):
        if word[l] == word[l+1] and word[l+3] == word[l+4] and word[l+5] == word[l+6]:
            return True
    return False           
'''
does the splitter have to be different from the splitees?
if so change function
'''

def has_three_double_split2(word):
...
for word in word_file:
    ...
    if has_two_double(word):
        print(word)
        count = count + 1
    elif has_three_double(word):
        print(word)
        count = count + 1
    elif has_three_double_split1(word):
        print(word)
        count = count + 1
    elif has_three_double_split2(word):
        print(word)
        count = count + 1        
